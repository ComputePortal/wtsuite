...
func searchSVGStrokeWidth(scope Scope, attr *tokens.StringDict,
	ctx context.Context) (*tokens.Float, error) {
	swToken_, err := SearchStyle(scope, attr, "stroke-width", ctx)
	if err != nil {
		return nil, err
	}
...
func buildSVGArrow(scope Scope, node Node, tag *tokens.Tag) error {
	...
	sw, err := searchSVGStrokeWidth(scope, attr, tag.Context())
	if err != nil {
		return err
	}
