...
func (t *HTML) getHeadBody() (*Head, *Body, error) {
	...
	for _, child := range t.children {
		...
		default:
			...
			return nil, nil, errCtx.NewError("HTML Error: expected head or body")
		}
	}
...
func (t *HTML) collectAutoHrefRule(ss styles.Sheet) error {
	if !AUTO_HREF {
		return nil
	}
...
func (t *HTML) CollectStyles(idMap IDMap, classMap ClassMap, cssUrl string) ([][]string, error) {
	...
	if err := t.collectAutoHrefRule(ss); err != nil {
		return nil, err
	}
